...
if __name__=='__main__':
	...
	#taking only a subset of the data, as my laptop will burn to the ground otherwise
	...
	#Random backup name, change at will, it will get a pickle of the autoencoder weight and bias

	#set values to something useful, batch_size and number of epochs doesn't seem to make much of a difference
	training_epochs = 300
	...
	#n_filters is 30 so I can run it on my laptop, reasonable values should be around 100, 200 at least.
	n_filters = 200
	
	output_file = 'ae_'+str(patch_size)+'x'+str(patch_size)+'_'+str(n_filters)+'_filters_backup.pikle'
	...
	#if the autoencoder hasn't been trained and pickled beforehand, train it and back it up now
...
'''
		...
		#if autoencoder has been trained and backed up in the file named, load it up from there
	...
	errors_train = theano.function([index],err_train,givens=[(y,train_y[index]),(inp,train_x[index])])
	errors_test = theano.function([],err_test,givens = [(val_y,validation_y),(val_inp,validation_x)])
	...
	for epoch in xrange(training_epochs):
		...
		print 'LR training epoch %d, train cost %lf, training error %lf, validation cost %lf, validation error %lf' % (epoch, np.mean(c), train_e, ve, test_e)
	print '---->\n.....Dumping logistic regression parameters to log_reg_params.pickle'	
